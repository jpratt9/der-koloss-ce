...
// modules in js/weapons/ that it re-exports. The stylesheet is style.css plus
// the files in style/, which index.html links in name order. A check reads all
// of one as a single string, so it keeps passing when the text it pins moves
// between files.
