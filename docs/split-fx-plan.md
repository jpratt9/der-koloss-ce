...
- **The standing check from §1.7 was added after the split,** as scripts/test-fx-effects.mjs, with the fakes it needs in scripts/lib/headless-fx.mjs. That makes 64 validators, and js/fx.js and js/fx/ the second thing in js/ that a validator now runs end to end.
  - 27 checks: the five surfaces with their own dust, chips, sparks, hole colour and light, and an unknown surface falling back to concrete; blood spray, mist and a floor pool in the pinned blood colour; the decal, shell, tracer and popup rings wrapping instead of growing; a tracer under the 0.25 m floor dropped, a long one billboarded and handed back to the scene graph when it retires; a case that bounces at most three times and clicks exactly once; a muzzle flash that spikes and is dark four frames later; blasts that scale with radius, shake, and never push trauma past 1; a bowed arc pinned at both ends; the DOM and post flash paths never both firing, and the vignette cancelling its own timer; drops born with a hidden light that bob, blink and leave the scene; and a teardown that empties every pool and ring.
  - It was run on copies of the tree with seven broken behaviours, and failed on each: metal that stops sparking, a blood colour that drifts, a decal ring that stops wrapping, brass that clicks on every bounce, a vignette that stops cancelling its timer, a drop light born visible, and a teardown that stops emptying the pools.
