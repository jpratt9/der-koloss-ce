...
scripts/            69 headless validators
about/  api/        About page; invite-link OG metadata
```

## Validators

The project has no unit tests. It has **69 headless validators** that assert gameplay and rendering invariants — things that are expensive to rediscover once broken: control-key collisions, camera-spring stability, allocation budgets in the collision and particle hot paths, coplanar geometry, spawn sources, shot occlusion, weapon model integrity, the shared geometry and material libraries behind the view-models, the seeded surfaces every machine is built from, the pooled co-op uniforms and corpse damage, host/guest combat agreement, a whole co-op session against a fake PeerJS, and every effect the game draws against recording particle pools.
